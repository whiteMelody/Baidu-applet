...
App({
  ...
  //存储字体大小颜色
  setFont(list) {
    swan.setStorageSync('font', JSON.stringify(list))
    return true
  },

  //获取字体大小颜色
  getFont() {
    let res = swan.getStorageSync('font')
    if (this.isNull(res)) {
      return []
    } else {
      return JSON.parse(res)
  ...
  //存储搜索历史
  setHistory(list) {
    swan.setStorageSync('search', JSON.stringify(list))
    return true
  },

  //获取搜索历史
  getHistory() {
    let res = swan.getStorageSync('search')
    if (this.isNull(res)) {
      return []
    } else {
      return JSON.parse(res)
